...
export const AutoDetailUsado = () => {
  ...
  useEffect(() => {
    const obtenerDetallesAuto = async () => {
      const accessToken = localStorage.getItem('accessToken'); // Obtener el Access Token de localStorage
      if (accessToken) {
        try {
    ...
    <div className="auto-detail-container">
      <div className="auto-detail-card">
        ...
        <div className="auto-info">
          ...
          <p><strong>Publicado por:</strong> {auto.user ? auto.user.username : 'Usuario desconocido'}</p>
        </div>
