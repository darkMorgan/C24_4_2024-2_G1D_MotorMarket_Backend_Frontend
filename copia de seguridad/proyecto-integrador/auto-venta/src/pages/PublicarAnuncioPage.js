...
const PublicarAnuncioPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    const { marca, modelo, year, precio } = formData;
    if (!marca || !modelo || !year || !precio) {
    ...
    dataToSend.append(
      'auto',
      JSON.stringify({ marca, modelo, year: parseInt(year), precio: parseFloat(precio) })
    );
    if (imagen) {
    ...
    <div className="container mt-5">
      <div className="card shadow mx-auto p-4" style={{ maxWidth: '500px' }}>
        ...
        <form onSubmit={handleSubmit}>
          {/* Campos de texto */}
          {['marca', 'modelo', 'year', 'precio'].map((field, index) => (
